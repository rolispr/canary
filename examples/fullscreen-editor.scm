...
(define-class <model> ()
  ...
  (term-width #:init-value 80 #:accessor term-width)
  (term-height #:init-value 24 #:accessor term-height))
...
(define (update m msg)
  (cond
   ...
   ((is-a? msg <window-size-msg>)
    (set! (term-width m) (slot-ref msg 'width))
    (set! (term-height m) (slot-ref msg 'height))
    (values m #f))
...
(define (view m)
  (let* ((w (term-width m))
         ...
         ;; Use full height minus title, spacing, and status (5 lines)
         (content-h (max 1 (- h 5)))
         ;; Use full width minus border (4 chars)
    ...
    (vbox
     (pad (align (txt "Text Editor" #:bold? #t #:fg "#ff6b9d") 'center) #:bottom 1)
...
(define model (make <model>
                #:lines '("Welcome to the full-screen text editor!"
